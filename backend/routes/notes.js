...
// router.delete('/deletenote/:uid', fetchuser, async (req, res) => {
//     try {
//         //Find the note to be updated and update it
//         let note = await Note.remove({uid: req.params.uid})
...
//ROUTE 5: Delete all items from cart using: DELETE "/api/note/deletecart". login required
router.delete('/deletecart/:user', fetchuser, async (req, res) => {
    try {
        //Find the note to be updated and update it
        let cart = await Note.remove({user: req.params.user})
        // let note = await Note.findOne({uid: req.params.uid});
